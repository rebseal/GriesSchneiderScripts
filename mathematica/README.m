(* ****************************************************************************
   ...
   The distinguishing feature of mathDragon from other proof assistants, theorem
   provers, and implementations of TLA is that it is also a general-purpose
   symbolic and numerical programming language, and a very good one. We want to
   expose a programming language in the style of Mathematica as a text-to-text
   library that can be embedded in any kind of programming tool or environment,
   even just a raw terminal, but certainly including emacs; IDEs like Eclipse,
   IntelliJ, and Visual Studio Code; cloud apps; and web apps. No other proof
   assistant, theorem prover, or implementation of TLA to our knowledge embodies
   the combination of (1) full integration with general-purpose programming, (2)
   open source, and (3) basis on a text-to-text core library that can be
   embedded in any other program.
   ...
   The files in the "mathematica" directory are baselines for comparing other
   versions tailored for mathics and for SymJa. They are back-ports from mathics
   to Mathematica.
   ...
   matcher, according to a Wolfram insider. That means he didn't know
   off-the-cuff exactly why the pattern-matcher produces a certain answer for my
   edge case. I also learned that there have been bugs in the pattern-matcher as
   late as Mathematica 10.1. I have been using Mathematica every day for 35
   years, and I am still learning things about the pattern-matcher. I certainly
   did while pursuing differences amongst Mathematica, mathics, and SymJa.
   ...
   with the Flat attribute and combinations of the OneIdentity and Orderless
   attributes. Mathematica and mathics sometimes differ in the default answer
   that "ReplaceAll" gives. To get all the correct answers, use "ReplaceList."
   To get the desired answer, pick it out of the resulting list using [[part]]
   ...
   ****************************************************************************
   ...
   | (_ | '_| / -_|_-< > _|_ _| \__ \/ _| ' \| ' \/ -_) / _` / -_) '_|
    ...
    method. Formal methods means "machine-checked calculations and proofs."
    Formal Methods help you write better software. They can help you avoid
    billion-dollar mistakes, like crashing the Mars Climate Observer because the
    units of measure "newton" and "pound-force" were not checked by machine.
    Like losing customer data in a cloud database because of an unanticipated
    edge-case thirty-five steps into a leader-election protocol.
    ...
    (https://goo.gl/dx32Mw). Statecharts allowed me to formally prove by
    exhaustive testing that an embedded controller for a robot had no bugs. TLA+
    saved Amazon's Dynamo DB a catastrophic failure (https://goo.gl/pTpZYT).
    Many mistakes have been found in published algorithms and protocols at the
    foundational layer of the internet and cloud computing when those protocols
    were subjected to formal methods (no citation).
